...
function markWords(string, words) {
  if (words) {
    for (var i = 0; i < words.length; i++) {
      string = words[i].length
        ? string.replace(new RegExp('(' + words[i] + ')', 'gi'), MARK_CHAR + '$1' + MARK_CHAR)
        : string
    }
  }
        ...
        replacePlaceholders(
          metaListItemTemplate,
          {
            ...
            value: metaData[i].valueLink
              ? '<a href="' +
                metaData[i].valueLink +
                '" target="_blank">' +
                convertMarksToHtml(htmlEncode(markWords(value, valueMatches))) +
                '</a>'
            ...
            attributes: function () {
              ...
              for (var name in metaData[i].attributes) {
                if (metaData[i].attributes.hasOwnProperty(name)) {
                  attributes.push(
                    replacePlaceholders(metaListItemAttributeTemplate, {
                      name: name,
...
function inspectInElementsPanel(idx) {
  chrome.devtools.inspectedWindow.eval(
    "inspect($$('head title, head base, head meta, head link')[" + parseInt(idx) + '])'
  )
}
